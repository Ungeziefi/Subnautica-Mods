...
#pragma warning disable Harmony003 // Harmony non-ref patch parameters modified
        var worldPos = camera.ViewportToWorldPoint(
...
#pragma warning restore Harmony003 // Harmony non-ref patch parameters modified
        ...
        var dirToResource = worldPos - camera.transform.position;
        dirToResource.Normalize();

        // Project onto camera plane
        var rightDot = Vector3.Dot(dirToResource, camRight);
        var upDot = Vector3.Dot(dirToResource, camUp);
        ...
        if (Main.Config.UseCircularEdgeBlips)
        {
            ...
            var maxRadius = 0.5f - Main.Config.EdgeMargin / minScreenDim;
            var radius = Mathf.Min(Main.Config.CircleRadius / 100f, maxRadius);
        ...
        else
        {
            ...
            var marginX = Main.Config.EdgeMargin / Screen.width;
            var marginY = Main.Config.EdgeMargin / Screen.height;
        ...
        if (!string.IsNullOrEmpty(resourceId))
        {
            ...
            // Apply smoothing
            if (lastPositions.TryGetValue(resourceId, out var prevPos))
