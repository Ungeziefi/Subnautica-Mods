...
public class KnownFragmentFilter
{
    ...
    {
        return !string.IsNullOrEmpty(uniqueId) && FragmentTypeCache.TryGetValue(uniqueId, out var type)
            ? type
            : TechType.None;
    }
    ...
    {
        var fragmentType = GetFragmentType(uniqueId);
    ...
    private static bool IsFragmentForKnownBlueprint(TechType fragmentType)
    {
        ...
        var blueprint = PDAScanner.GetEntryUnlockable(fragmentType, out _);
        return KnownTech.Contains(blueprint);
    }

    // Capture resource registration to cache fragment types
    [HarmonyPatch(typeof(ResourceTracker), nameof(ResourceTracker.Register))]
    [HarmonyPostfix]
    public static void ResourceTracker_Register(ResourceTracker __instance)
    {
        ...
        if (techType != TechType.None && techType != TechType.Fragment) FragmentTypeCache[uniqueId] = techType;
    }
}