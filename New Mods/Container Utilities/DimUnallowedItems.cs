...
public class DimUnallowedItems
{
    ...
    public static void OnOpenPDA(uGUI_InventoryTab __instance)
    {
        ...
        for (var i = 0; i < Inventory.main.usedStorage.Count; i++)
        {
        ...
        foreach (var pair in __instance.inventory.items)
        {
            ...
            var techType = invItem.item.GetTechType();
            var itemType = TechData.GetEquipmentType(techType);

            if (!container.AllowedToAdd(pickupable, false) ||
                (container is Equipment equipment && !equipment.GetCompatibleSlot(itemType, out _))) icon.SetChroma(0f);
        }
    }

    // Reset
    [HarmonyPatch(typeof(uGUI_InventoryTab), nameof(uGUI_InventoryTab.OnClosePDA))]
    [HarmonyPostfix]
