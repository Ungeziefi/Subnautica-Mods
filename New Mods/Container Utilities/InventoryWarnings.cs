...
public class InventoryWarnings
{
    ...
    private static readonly FieldInfo itemsMapField =
        typeof(ItemsContainer).GetField("itemsMap", BindingFlags.NonPublic | BindingFlags.Instance);
    ...
    private static void UpdateWarnings(Inventory inventory)
    {
        ...
        var freeSlots = CalculateFreeSlots(container);
        var isFullNow = freeSlots == 0;
        ...
        {
            var slotText = freeSlots == 1 ? "slot" : "slots";
    ...
    private static int CalculateFreeSlots(ItemsContainer container)
    {
        ...
        for (var y = 0; y < container.sizeY; y++)
        for (var x = 0; x < container.sizeX; x++)
