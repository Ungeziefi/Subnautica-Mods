...
public class CuddlefishRenamer
{
    ...
    private static bool isRenamingActive;
    ...
    private static void UpdateAllNameLabelsAppearance()
    {
        foreach (var label in nameLabels.Values)
        {
            ...
            if (text != null)
            {
                ...
                text.fontStyle = Main.Config.BoldText ? FontStyles.Bold : FontStyles.Normal;
            }
        }
    }
    ...
    public static void CuteFish_Start(CuteFish __instance)
    {
        ...
        var cuddlefishId = GetCuddlefishId(__instance);
        if (TryGetSavedName(cuddlefishId, out var savedName) && Main.Config.ShowNameAbove)
    ...
    {
        var oldText = HandReticle.main.textHandSubscript;

        // Update play prompt with custom name
        if (Main.Config.UseNameInPlayPrompt)
        {
            var cuddlefishId = GetCuddlefishId(handTarget.cuteFish);
            if (TryGetSavedName(cuddlefishId, out var savedName))
                HandReticle.main.SetText(HandReticle.TextType.Hand, $"Play With {savedName}", false,
                    GameInput.Button.LeftHand);
        }

        // Add rename prompt
        var renameText = GameInput.FormatButton(Main.RenameCuddlefishButton);
    ...
    private static void CheckRenameInput(CuteFishHandTarget handTarget)
    {
        ...
        var cuddlefish = handTarget.cuteFish;
        if (cuddlefish == null || !handTarget.liveMixin.IsAlive()) return;

        var cuddlefishId = GetCuddlefishId(cuddlefish);
        if (string.IsNullOrEmpty(cuddlefishId)) return;

        TryGetSavedName(cuddlefishId, out var currentName);
    ...
    private static void UpdateAllNameLabelsVisibility()
    {
        ...
        foreach (var label in nameLabels.Values)
        {
            ...
            {
                var cuddlefish = label.GetComponentInParent<CuteFish>();
                if (cuddlefish != null)
                {
                    var distance = Vector3.Distance(playerPosition, cuddlefish.transform.position);
    ...
    {
        var text = labelObj.GetComponentInChildren<TextMeshProUGUI>();
        if (text == null) return;

        var maxDistance = fadeStartDistance * 2;
        ...
        {
            var fadeAmount = (distance - fadeStartDistance) / fadeStartDistance;
    ...
    private static void SetCuddlefishName(CuteFish cuddlefish, string cuddlefishId, string newName)
    {
        if (string.IsNullOrEmpty(newName))
        {
            ...
            if (nameLabels.TryGetValue(cuddlefishId, out var label) && label != null)
            {
        ...
        else
        {
            ...
            if (Main.Config.ShowNameAbove && cuddlefish != null) UpdateNameLabel(cuddlefish, newName);
        }
    }
    ...
    private static void UpdateNameLabel(CuteFish cuddlefish, string name)
    {
        ...
        if (!nameLabels.TryGetValue(cuddlefishId, out var labelObj) || labelObj == null)
    ...
    private static void UpdateInitialVisibility(GameObject labelObj, CuteFish cuddlefish)
    {
        ...
        if (Main.Config.FadeWithDistance)
        {
            var distance = Vector3.Distance(Player.main.transform.position, cuddlefish.transform.position);
    ...
    private static void ApplyTextFormatting(TextMeshProUGUI text)
    {
        text.color = Main.Config.NameColor;
        text.fontStyle = Main.Config.BoldText ? FontStyles.Bold : FontStyles.Normal;
    }
    ...
    {
        var identifier = cuddlefish.GetComponent<UniqueIdentifier>();
