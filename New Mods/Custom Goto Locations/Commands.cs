...
public partial class CustomGoto
{
    ...
    private void OnConsoleCommand_addgoto(NotificationCenter.Notification n)
    {
        ...
        var locationName = (string)n.data[0];
    ...
    private void OnConsoleCommand_renamegoto(NotificationCenter.Notification n)
    {
        ...
        var oldName = (string)n.data[0];
        var newName = (string)n.data[1];
        var location = FindLocation(oldName);
    ...
    private void OnConsoleCommand_listgoto(NotificationCenter.Notification n)
    {
        ...
        var filter = GetFilterFromNotification(n);

        // List both vanilla and custom locations
        var hasLocations = ListVanillaLocations(filter);
        hasLocations |= ListCustomLocations(filter);

        if (!hasLocations) ErrorMessage.AddMessage(string.Format(Messages.NoLocationsMatching, filter));
    }
    ...
    private void OnConsoleCommand_customgoto(NotificationCenter.Notification n)
    {
        HandleCustomGoto(n, false);
    }

    private void OnConsoleCommand_cgoto(NotificationCenter.Notification n)
    {
        HandleCustomGoto(n, false);
    }

    private void OnConsoleCommand_customgotofast(NotificationCenter.Notification n)
    {
        HandleCustomGoto(n, true);
    }

    private void OnConsoleCommand_cgotofast(NotificationCenter.Notification n)
    {
        HandleCustomGoto(n, true);
    }
