...
public partial class CustomGoto
{
    ...
    {
        var hasVanillaMatches = false;
        var hasCustomMatches = false;
        ...
        // Display results
        if (hasVanillaMatches) ErrorMessage.AddMessage(Messages.VanillaLocationsMatching + vanillaSb);
        if (hasCustomMatches) ErrorMessage.AddMessage(Messages.CustomLocationsMatching + customSb);
    }
