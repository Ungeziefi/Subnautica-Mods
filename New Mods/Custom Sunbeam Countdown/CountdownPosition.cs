...
public class CustomSunbeamCountdown
{
    ...
    private static bool originalValuesStored;

    [HarmonyPatch(typeof(uGUI_SunbeamCountdown), nameof(uGUI_SunbeamCountdown.UpdateInterface))]
    [HarmonyPostfix]
    private static void uGUI_SunbeamCountdown_UpdateInterface(uGUI_SunbeamCountdown __instance)
    {
        var rectTransform = __instance.countdownHolder.GetComponent<RectTransform>();
        ...
        if (Main.Config.EnableFeature)
        {
            ...
            // Apply settings
            var xPos = Main.Config.XPosition;
            var yPos = Main.Config.YPosition;
            var scale = Main.Config.Scale;
            var margin = 10f;
