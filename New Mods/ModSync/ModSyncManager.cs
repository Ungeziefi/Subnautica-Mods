...
public static class ModSyncManager
{
    ...
    public static void SaveModList(string savePath)
    {
        try
        {
            var currentMods = GetCurrentModList();
            var filePath = Path.Combine(savePath, ModSyncFileName);
            var json = JsonConvert.SerializeObject(currentMods, Formatting.Indented);
    ...
    public static ModComparison CompareModLists(ModListData savedMods, ModListData currentMods)
    {
        ...
        var savedDict = savedMods.Mods.ToDictionary(m => m.Name);
        var currentDict = currentMods.Mods.ToDictionary(m => m.Name);
        ...
        foreach (var saved in savedMods.Mods)
            if (!currentDict.TryGetValue(saved.Name, out var current))
                comparison.Removed.Add(saved);
            else if (saved.Version != current.Version) comparison.VersionChanged.Add((saved, current));
    ...
    public static string FormatModComparison(ModComparison comparison)
    {
        ...
        if (comparison.Added.Count > 0)
        {
            ...
            foreach (var mod in comparison.Added) message.AppendLine($"{mod.Name} (v{mod.Version})");
        ...
        if (comparison.Removed.Count > 0)
        {
            ...
            foreach (var mod in comparison.Removed) message.AppendLine($"{mod.Name} (v{mod.Version})");
            message.AppendLine(new string('\u2500', 34));
        }
