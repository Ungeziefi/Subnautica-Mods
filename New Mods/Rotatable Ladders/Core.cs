...
public partial class RotatableLadders
{
    ...
    public static void BaseLadder_OnHandHover(BaseLadder __instance)
    {
        ...
        var isHoldingItem = Inventory.main.GetHeldTool() != null;
        if (Main.Config.EmptyHandsOnly && isHoldingItem) return;

        var rotateText = GameInput.FormatButton(Main.RotateLadderButton);
        ...
        if (GameInput.GetButtonDown(Main.RotateLadderButton))
        {
            ...
            var coords = GetLadderCoords(parent);
            var newYRotation = (parent.localRotation.eulerAngles.y + 90f) % 360f;
    ...
    public static void BaseLadder_Start(BaseLadder __instance)
    {
        ...
        // Apply rotation
        if (dictionary.TryGetValue(coords, out var savedRotation))
