...
public class PRAWNParticlesOnlyOnSand
{
    ...
    private static bool ShouldPlayJumpFX_ApplyJumpForce(bool onGround, Exosuit exosuit)
    {
        ...
        if (Physics.Raycast(new Ray(exosuit.transform.position, Vector3.down), out var hitInfo, 10f))
        {
    ...
    [HarmonyPatch(typeof(Exosuit), nameof(Exosuit.ApplyJumpForce))]
    [HarmonyTranspiler]
    private static IEnumerable<CodeInstruction> Exosuit_ApplyJumpForce(IEnumerable<CodeInstruction> instructions,
        ILGenerator generator)
    {
        ...
        matcher.Advance(1).Insert(
            ...
            new CodeInstruction(OpCodes.Call,
                AccessTools.Method(typeof(PRAWNParticlesOnlyOnSand), nameof(ShouldPlayJumpFX_ApplyJumpForce)))
        );
    ...
    [HarmonyPatch(typeof(Exosuit), nameof(Exosuit.OnLand))]
    [HarmonyTranspiler]
    private static IEnumerable<CodeInstruction> Exosuit_OnLand(IEnumerable<CodeInstruction> instructions,
        ILGenerator generator)
    {
        var matcher = new CodeMatcher(instructions, generator);
        ...
        matcher.Start().Insert(
            ...
            new CodeInstruction(OpCodes.Call,
                AccessTools.Method(typeof(PRAWNParticlesOnlyOnSand), nameof(ShouldPlayJumpFX_OnLand))),
