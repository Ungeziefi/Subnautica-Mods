...
public class SaveCyclopsClosedDoors
{
    ...
    private static void CloseDoor(Openable openable)
    {
        ...
        if (closedDoorsYAngle.TryGetValue(openable.name, out var y))
        {
            var rot = openable.transform.localEulerAngles;
            openable.transform.localEulerAngles = new Vector3(rot.x, y, rot.z);
        }
    }

    [HarmonyPatch(typeof(Openable), nameof(Openable.OnHandClick))]
    [HarmonyPostfix]
    ...
    [HarmonyPatch(typeof(Openable), nameof(Openable.Start))]
    [HarmonyPrefix]
