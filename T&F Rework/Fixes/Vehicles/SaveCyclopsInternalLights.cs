...
public class SaveCyclopsInternalLights
{
    private static string GetCyclopsId(CyclopsLightingPanel panel)
    {
        ...
        var gameObject = panel.cyclopsRoot.gameObject;
        if (gameObject == null)
            return null;
    ...
    [HarmonyPatch(typeof(CyclopsLightingPanel), nameof(CyclopsLightingPanel.ToggleInternalLighting))]
    [HarmonyPostfix]
    ...
    // Load state
    [HarmonyPatch(typeof(CyclopsLightingPanel), nameof(CyclopsLightingPanel.Start))]
    [HarmonyPostfix]
