...
public class Config : ConfigFile
{
    ...
    [Toggle(Label = "Chair swivelling")] public bool ChairSwivelling = false;
    ...
    [Toggle(Label = "Power cell charge from batteries",
        Tooltip =
    ...
    [Toggle(Label = "Cyclops displays needs power",
        Tooltip = "Disables various screens and systems when the power is off.")]
    public bool CyclopsDisplaysNeedPower = false;

    [Toggle(Label = "Batteries have tooltips")]
    public bool BatteriesHaveTooltips = false;

    [Toggle(Label = "No PDA delay")] public bool NoPDADelay = false;
    ...
    [Toggle(Label = "Moveable Mobile Vehicle Bay",
        Tooltip = "Allows the repulsion and propulsion cannons to move the Mobile Vehicle Bay.")]
    ...
    [Toggle(Label = "No obstacle check when sleeping",
        Tooltip = "Allows sleeping even if there's an object in the way.")]
    ...
    [Toggle(Label = "No floodlights on build",
        Tooltip = "Stops the floodlights from being defaulted to on when a Cyclops is built.")]
    public bool NoFloodlightsOnBuild = false;

    [Toggle(Label = "Escape closes PDA")] public bool EscapeClosesPDA = false;

    [Slider(Label = "Seaglide light angle",
        Tooltip =
            "The angle in degrees to tilt the Seaglide light beam upward (higher value) or downward (lower value).",
    ...
    [Toggle(Label = "Passive engine overheating",
        Tooltip = "The Cyclops engine can now overheat even when the throttle is not applied.")]
    ...
    [Toggle(Label = "Upgrade only when docked",
        Tooltip = "Prevents opening the Seamoth's or PRAWN Suit's upgrade panel unless docked.")]
    public bool UpgradeOnlyWhenDocked = false;

    [Toggle(Label = "Disable swim warning")]
    public bool DisableSwimWarning = false;

    [Toggle(Label = "Disable oxygen warning")]
    public bool DisableOxygenWarning = false;

    [Toggle(Label = "No Scanner icon if scanned",
        Tooltip = "Prevents the scanner icon from displaying on fragments for known recipes.")]
    public bool NoScannerIconIfScanned = false;

    [Toggle(
        "<color=#FFAC09FF>Torpedo cycling</color> <alpha=#00>----------------------------------------------------------------------------</alpha>")]
    public bool TorpedoCyclingDivider;

    [Toggle(Label = "Enable feature")] public bool TCEnableFeature = false;

    [Toggle(
        "<color=#FFAC09FF>Animated locker doors</color> <alpha=#00>----------------------------------------------------------------------------</alpha>")]
    ...
    [Slider(Label = "Locker door animation duration", DefaultValue = 0.5f, Min = 0.1f, Max = 1f, Step = 0.1f,
        Format = "{0:0.0}s")]
    public float LockerDoorAnimationDuration = 0.5f;

    [Slider(Label = "Single door opening angle", DefaultValue = 135f, Min = 15f, Max = 180f, Step = 5f,
        Format = "{0:0}°")]
    public float SingleDoorOpenAngle = 135f;

    [Slider(Label = "Double door opening angle", DefaultValue = 90f, Min = 15f, Max = 135f, Step = 5f,
        Format = "{0:0}°")]
    public float DoubleDoorOpenAngle = 90f;

    [Toggle(
        "<color=#FFAC09FF>Toggle PRAWN Suit lights</color> <alpha=#00>----------------------------------------------------------------------------</alpha>")]
    public bool TogglePRAWNSuitLightsDivider;

    [Toggle(Label = "Enable feature")] public bool TPSLEnableFeature = false;

    [Toggle(
        "<color=#FFAC09FF>Base lights</color> <alpha=#00>----------------------------------------------------------------------------</alpha>")]
    public bool BaseLightsDivider;

    [Toggle(Label = "Toggle base lights",
        Tooltip = "Allows toggling the lights in the current base. Keybind configurable in the \"Mod Input\" menu.")]
    public bool ToggleBaseLights = false;

    [Slider(Label = "Toggle hold duration",
        Tooltip = "The duration (in seconds) you need to hold the key to toggle base lights.", DefaultValue = 2,
        Min = 1, Max = 4, Step = 1, Format = "{0}s")]
    public int ToggleHoldDuration = 2;

    [Toggle(Label = "Toggle lights for sleep",
        Tooltip = "Automatically toggles the lights when going to bed and waking up.")]
    public bool ToggleLightsForSleep = false;

    [Slider(Label = "Lights on after sleep delay",
        Tooltip = "Delay (in seconds) before base lights turn back on after sleeping.", DefaultValue = 10, Min = 1,
        Max = 10, Step = 1, Format = "{0}s")]
    public int LightsOnAfterSleepDelay = 10;

    [Toggle(
        "<color=#FFAC09FF>Geysers push objects</color> <alpha=#00>----------------------------------------------------------------------------</alpha>")]
    public bool GeysersPushObjectsDivider;

    [Toggle(Label = "Geysers push objects")]
    public bool GPOEnableFeature = false;

    [Slider(Label = "Eruption force",
        Tooltip = "The force applied to objects during geyser eruptions. Higher values push objects farther.",
        DefaultValue = 50f, Min = 0f, Max = 200f, Step = 5f)]
    public float GeyserEruptionForce = 50f;

    [Toggle(
        "<color=#FFAC09FF>Miscellaneous</color> <alpha=#00>----------------------------------------------------------------------------</alpha>")]
    public bool MiscellaneousTweaksDivider;

    [Toggle(Label = "Bladderfish tooltip",
        Tooltip = "Adds a tooltip about the Bladderfish providing oxygen if consumed raw.")]
    public bool BladderfishTooltip = false;

    [Toggle(Label = "\"Use Trashcan\" -> \"Use Trash can\"",
        Tooltip = "Adds a missing space to the Trashcan prompt for consistency with its recipe name.")]
    public bool TrashcanNameConsistency = false;

    [Toggle(Label = "Capitalize \"Use\"",
        Tooltip = "Capitalizes the \"Use\" prompt for consistency with other prompts.")]
    public bool CapitalizeUseString = false;

    [Toggle(
        "<color=#FFAC09FF>Multipliers</color> <alpha=#00>----------------------------------------------------------------------------</alpha>")]
    public bool MultipliersDivider;

    [Slider(Label = "Build time multiplier",
        Tooltip = "Higher values make building take longer. Lower values make building faster.",
        DefaultValue = 1f, Min = 0.01f, Max = 10f, Step = 0.01f, Format = "{0:0.00}x")]
    public float BuildTimeMultiplier = 1f;

    [Slider(Label = "Craft time multiplier",
        Tooltip = "Higher values make crafting take longer. Lower values make crafting faster.",
        DefaultValue = 1f, Min = 0.01f, Max = 10f, Step = 0.01f, Format = "{0:0.00}x")]
    public float CraftTimeMultiplier = 1f;

    [Slider(Label = "Plant growth time multiplier",
        Tooltip = "Higher values make plants grow slower. Lower values make plants grow faster.",
        DefaultValue = 1f, Min = 0.01f, Max = 10f, Step = 0.01f, Format = "{0:0.00}x")]
    ...
    [Toggle(
        "<color=#FFAC09FF>Creature size randomizer</color> <alpha=#00>----------------------------------------------------------------------------</alpha>")]
    public bool CreatureSizeRandomizerDivider;

    [Toggle(Label = "Enable feature")] public bool CSREnableFeature = false;
    ...
    [Slider(Label = "Minimum multiplier", DefaultValue = 0.75f, Min = 0.01f, Max = 1f, Step = 0.01f,
        Format = "{0:0.00}x")]
    ...
    [Toggle(
        "<color=#FFAC09FF>New commands</color> <alpha=#00>----------------------------------------------------------------------------</alpha>")]
    public bool NewCommandsDivider;

    [Toggle(Label = "RestoreHealth")] public bool RestoreHealth = false;

    [Toggle(Label = "RestoreHunger")] public bool RestoreHunger = false;

    [Toggle(Label = "RestoreThirst")] public bool RestoreThirst = false;

    [Toggle(Label = "RestoreAll")] public bool RestoreAll = false;

    [Toggle(Label = "QQQ", Tooltip = "Quits the game without confirmation.")]
    public bool QQQ = false;

    [Toggle(
        "<color=#FFAC09FF>Day/night duration</color> <alpha=#00>----------------------------------------------------------------------------</alpha>")]
