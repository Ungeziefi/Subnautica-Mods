...
public class AnimatedLockerDoors
{
    ...
    private const string LargeLockerLeftDoorPath =
        "model/submarine_Storage_locker_big_01/submarine_Storage_locker_big_01_hinges_L";

    private const string LargeLockerRightDoorPath =
        "model/submarine_Storage_locker_big_01/submarine_Storage_locker_big_01_hinges_R";
    ...
    public class LockerDoorOpener : MonoBehaviour
    {
        ...
        public IEnumerator Rotate(Transform door, bool playCloseSound = false)
        {
            while (t < duration)
            {
                ...
                var rotation = Mathf.Lerp(startRotation, endRotation, progress);
                ...
                if (endRotation == 0f)
                {
                    if (progress >= 0.6f && playCloseSound && closeSound != null)
                    {
                        ...
                        FMODUWE.PlayOneShot(closeSound, door.transform.position);
                    }
                    else if (progress >= 1f)
                    {
                        ReparentLabel(door);
        ...
        public IEnumerator Rotate(Transform doorLeft, Transform doorRight, bool playCloseSound = false)
        {
            while (t < duration)
            {
                ...
                doorLeft.localEulerAngles =
                    new Vector3(doorLeft.localEulerAngles.x, doorLeft.localEulerAngles.y, -rotation);
                doorRight.localEulerAngles =
                    new Vector3(doorRight.localEulerAngles.x, doorRight.localEulerAngles.y, rotation);

                if (progress >= 0.4f && playCloseSound && closeSound != null)
                {
                    playCloseSound = false;
                    FMODUWE.PlayOneShot(closeSound, doorLeft.transform.position);
                }
    ...
    private class StorageContainer_Patch
    {
        ...
        private static void OpenWallLocker(StorageContainer instance)
        {
            ...
            if (openSound != null)
                FMODUWE.PlayOneShot(openSound, instance.transform.position);
        }
        ...
        [HarmonyPostfix]
        private static void StorageContainer_Open(StorageContainer __instance, Transform useTransform)
        {
        ...
        [HarmonyPostfix]
        private static void StorageContainer_OnClose(StorageContainer __instance)
        {
