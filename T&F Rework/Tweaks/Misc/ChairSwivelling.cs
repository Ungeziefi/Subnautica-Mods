...
{
    private static Bench currentBench;
    ...
    private static float currentRotSpeed;
    private static int currentDirection; // 1 = right, -1 = left
    ...
    [HarmonyPatch(typeof(Bench), nameof(Bench.EnterSittingMode))]
    [HarmonyPostfix]
    private static void Bench_EnterSittingMode(Bench __instance)
    {
        currentBench = __instance;
    }

    [HarmonyPatch(typeof(Bench), nameof(Bench.ExitSittingMode))]
    [HarmonyPostfix]
    private static void Bench_ExitSittingMode(Bench __instance)
    {
    ...
    [HarmonyPatch(typeof(Bench), nameof(Bench.OnUpdate))]
    [HarmonyPostfix]
    public static void Bench_OnUpdate(Bench __instance)
    {
        ...
        // Subscript
        var moveRightText = GameInput.FormatButton(GameInput.Button.MoveRight);
        var moveLeftText = GameInput.FormatButton(GameInput.Button.MoveLeft);
        var gamepadMoveText = GameInput.FormatButton(GameInput.Button.Move);
