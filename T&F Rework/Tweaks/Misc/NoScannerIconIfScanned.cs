...
public class NoScannerIconIfScanned
{
    public static bool IsScanTargetBlueprintKnown()
    {
        ...
        var blueprint = PDAScanner.GetEntryUnlockable(scanTarget.techType, out _);
        return KnownTech.Contains(blueprint);
    }

    [HarmonyPatch(typeof(GUIHand), nameof(GUIHand.OnUpdate))]
    [HarmonyTranspiler]
    public static IEnumerable<CodeInstruction> GUIHand_OnUpdate(IEnumerable<CodeInstruction> instructions,
