...
public class SmokeClearsOnOpen
{
    [HarmonyPatch(typeof(EscapePodFirstUseCinematicsController),
        nameof(EscapePodFirstUseCinematicsController.OnTopHatchCinematicEnd))]
    [HarmonyPostfix]
    private static void EscapePodFirstUseCinematicsController_OnTopHatchCinematicEnd(EscapePod __instance)
    {
        ...
        if (lifePodDamagedFX != null)
            for (var i = 0; i < lifePodDamagedFX.childCount; i++)
            {
                var child = lifePodDamagedFX.GetChild(i);
                if (child.name.ToLower().Contains("smoke")) child.gameObject.SetActive(false);
            }
        ...
        if (radio != null) radio.gameObject.SetActive(false);
    }
}