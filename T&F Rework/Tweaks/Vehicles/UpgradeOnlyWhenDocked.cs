...
{
    [HarmonyPatch(typeof(VehicleUpgradeConsoleInput), nameof(VehicleUpgradeConsoleInput.OnHandHover))]
    [HarmonyPrefix]
    public static bool VehicleUpgradeConsoleInput_OnHandHover(SeamothStorageInput __instance)
    {
        ...
        if (vehicle != null && !vehicle.docked) return false;

        return true;
    }

    [HarmonyPatch(typeof(VehicleUpgradeConsoleInput), nameof(VehicleUpgradeConsoleInput.OnHandClick))]
    [HarmonyPrefix]
